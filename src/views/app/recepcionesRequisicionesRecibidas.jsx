...
import ContactListComponent from "components/recepcionesRequisicionesRecibidas/TableRecepciones";
import ContactWrapper from "components/recepcionesRequisicionesRecibidas/Recepciones.style";
import TableGroup from "components/recepcionesRequisicionesRecibidas/tableGroup";
import { remove, findIndex, filter } from "lodash";
import RecepcionesForm from "components/recepcionesRequisicionesRecibidas/RecepcionesForm";
...
class CargarUsuarios extends React.Component {
  ...
  async componentDidMount() {
    ...
    fetch(
      gsUrlApi +
        "/recepcionesrequisicionesrecibidas/Sede/" + Sede,
      {
        method: "GET",
        body: JSON.stringify(),
        headers: {
  ...
  ObtenerSelecionados = (data1, data2) => {
    this.setState((state) => ({
      ...state,
      SelectedPacientes: data1,
    }));
    this.setState((state) => ({
      ...state,
      SelectedMedicamentos: data2,
    }));
  };
  ...
  handleSearch = (e) => {
    ...
    this.setState((state) => ({
      ...state,
      listPacientes: filteredContactlists,
    }));
  };
  ...
  RecibirMedicamnetos = () => {
    ...
    for (let i = 0; i < this.state.listPacientes.length; i++) {
      ...
      if (ArrayPacientes.includes(this.state.listPacientes[i]._id)) {
        for (
          let k = 0;
          k < this.state.listPacientes[i].ArrayMedicamentos.length;
          k++
        ) {
          if (
            this.state.SelectedMedicamentos.includes(
              this.state.listPacientes[i].ArrayMedicamentos[k].Consecutivo
            )
          ) {
            Estado = true;
            this.state.listPacientes[i].ArrayMedicamentos[k].Estado =
              "Recibido";
            this.state.listPacientes[i].ArrayMedicamentos[
              k
            ].Cantidad = this.state.listPacientes[i].ArrayMedicamentos[
              k
            ].Pendiente;
          }
        }
  ...
  FiltraPacientes = (e) => {
    if (e.target) {
      ...
      fetch(
        gsUrlApi + "/dispensaciones_requisiciones/listarHistorial/" + Empresa,
        {
          method: "POST",
        ...
        .then((data) => {
          ...
          for (let i = 0; i < lstDatos.length; i++) {
            if (lstDatos[i].Sexo == "F") {
              lstDatos[i].LogoSexo = (
                <i className="fas fa-venus" style={{ color: "pink" }}></i>
              );
            } else {
              lstDatos[i].LogoSexo = (
                <i className="fas fa-mars" style={{ color: "blue" }}></i>
              );
            }
            if (lstDatos[i].FechaNacimiento) {
              let edad =
                fechaActual.substring(0, 4) -
                lstDatos[i].FechaNacimiento.substring(0, 4);
              if (
                fechaActual.substring(5, 7) <
                lstDatos[i].FechaNacimiento.substring(5, 7)
              ) {
                edad--;
              }
              if (
                ...
                fechaActual.substring(8, 10) >
                  lstDatos[i].FechaNacimiento.substring(8, 10)
              ) {
                edad--;
              }
              ...
              <div className="right-panel roe-box-shadow">
                <div className="contact-table">
                  ...
                  >
                    ...
                    this.state.listPacientes.length ? (
                      <TableGroup
                        ...
                        bodyLista={["", "Atc", "Descripcion", "Grupo", ""]}
                        Acciones={true}
                        CambiarEstadoRecibido={() =>
                          this.CambiarEstadoRecibido()
                        }
                        Consultar={(data) => this.Consultar(data)}
                        ObtenerSelecionados={(array1, array2) =>
                          this.ObtenerSelecionados(array1, array2)
                        }
                      ...
                      <div
                        className="text-center no-found-message"
                        style={{ color: "blue" }}
                      >
                        Cargando Datos....
                      </div>
