...
class Userlists extends React.Component {
    ...
    Filtrar = () => {
        ...
        fetch(gsUrlApi + '/dispensaciones/Informe', {
            method: 'POST',
            ...
            .then(data => data)
            .then((data) => {
                if (this.state.Aprovechamiento == false) {
                    let lstDatos = data.datos.filter((item) => {
                        if (item.Aprovechamiento > 0) {
                            if (item.CantidadProducto > item.Aprovechamiento) {
                                let Cantidad = item.CantidadProducto - item.Aprovechamiento
        ...
        return (
            <>
                ...
                <div className="row pb-3">
                    ...
                    <div className="col-md-2 mt-24 pretty cursor-pointer p-switch p-fill">
                        <input type="checkbox" checked={this.state.Aprovechamiento} className="cursor-pointer mt-24 " onChange={this.Aprovechamiento} />
                        <div className="state cursor-pointer mt-24 ">
                            <label>Incluir Aprovechamiento</label>
                        </div>
                    </div>
                    <div className=" col-md-2">
                                ...
                                <Table striped bordered hover className="mb-0" id={["Identificacion"]}>
                                    <thead className="">
                                        <tr className="border-bottom">
                                            ...
                                            <th>Codigo Bodega Usuario</th>
                                            <th>Nombre Bodega Usuario</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {this.state.ArrayDispensaciones.map((e, i) => {
                                            return (
                                                <tr key={i} className="border-bottom">
                                                    ...
                                                    <td>{e.CodigoEntrega}</td>
                                                    <td>{e.TipoDocumento + " - "}{e.IdentificacionPaciente}{" - "}{e.Paciente}</td>
                                                    <td>{e.Edad}</td>
                                                    <td>{e.NombreUnidadFuncional ? e.NombreUnidadFuncional : e.UnidadFuncional}</td>
                                                    <td>{e.Erp}</td>
                                                    ...
                                                    <td>{e.CodigoBodegaUsuario}</td> 
                                                    <td>{e.NombreBodegaUsuario}</td> 
                                                </tr>
                                            );
                                        })}
                                    </tbody>
