...
class RequisicionesForm extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.setState(state => ({
      ...state,
      BloqueoDispensacion: ObjSesion.Usuario?.Rol.Dispensacion
    }));
    this.setState(state => ({
      ...state,
      Sede: Sede.label
    ...
    this.setState(state => ({
      ...state,
      DataPaciente: this.props.values
    }));
      ...
      .then(data => {
        ...
        if (objData.length > 0) {
          ...
          for (let i = 0; i < objData.length; i++) {
            ...
            const exists = arrayTempP.some(
              item => item.value === objData[i].IdPrescripcion
            );
            if (!exists) {
              arrayTempP.push({
                value: objData[i].IdPrescripcion,
                label:
                  objData[i].IdPrescripcion +
                  " - " +
                  objData[i].CodigoUnidadFuncional +
                  " - " +
                  objData[i].UnidadFuncional
              });
            }
          }
          this.setState(state => ({
            ...state,
            arrayPrescripciones: arrayTempP
          }));
          this.setState(state => ({
            ...state,
            ListaPedido: objData
          ...
          let ObjSesion = JSON.parse(localStorage.getItem("Usuario"));
          this.listarBodegaPorSede(
            CodigoUnidadFuncional,
            ObjSesion?.Usuario?.Sede?.value
          );
        } else {
          this.setState(state => ({
            ...state,
    ...
    if (event.target) {
      ...
      if (event.target.checked === true) {
        if (listaTemp[event.target.id].ArrayMedicamentos.Cantidad === 1) {
          ...
          if (
            listaTemp[event.target.id].ArrayMedicamentos.Pendiente <
              event.target.value ||
        ...
        listaTemp[event.target.id].ArrayMedicamentos.Entrega =
          listaTemp[event.target.id].ArrayMedicamentos.Cantidad;
        listaTemp[event.target.id].ArrayMedicamentos.Pendiente = 0;
        listaTemp[event.target.id].ArrayMedicamentos.Aprovechamiento = "";
        listaTemp[
          event.target.id
    ...
    if (event.target) {
      ...
      if (event.target.checked === true) {
        if (listaTemp[event.target.id].Cantidad === 1) {
          ...
          if (
            listaTemp[event.target.id].Pendiente < event.target.value ||
            event.target.value < 1
          ) {
            this.AlertaValor();
          } else {
  ...
  ConfirmarMedicamento = () => {
    ...
    let CantidadUnidadEquivalente = this.state.ListaPedido[IndexMedicamento]
      .CantidadUnidadEquivalente;
    let Concentracion = this.state.ListaPedido[IndexMedicamento].Concentracion;

    this.setState(state => ({
      ...state,
      UnidadFuncionalImpr: this.state.ListaPedido[IndexMedicamento]
        .UnidadFuncional
    }));
    this.setState(state => ({
      ...state,
      CodigoUnidadFuncionalImpr: this.state.ListaPedido[IndexMedicamento]
        .CodigoUnidadFuncional
    }));

    if (
      CantidadUnidadEquivalente !== 0 &&
      CantidadUnidadEquivalente < 5 &&
      Concentracion < 5
    ) {
      ...
      if (Resultado !== 0) {
        ...
        this.AlertaCantidad(
          "El Medicamneto " +
            IdMedicamento +
            " - " +
            this.state.ListaPedido[IndexMedicamento].Generico +
            " Tiene una cantidad de " +
            Cantidad +
            " el cual deja aplicaciones incompletas"
        );
      } else {
        ...
        this.state.ListaPedido[IndexMedicamento].Pendiente =
          this.state.ListaPedido[IndexMedicamento].Cantidad - Cantidad;
      }
    } else {
      this.state.ListaPedido[
        IndexMedicamento
  ...
  Guardar = e => {
    ...
    if (this.state.EstadoGuardado === false) {
      ...
      objDisensacion.FechaPrescripcion =
        fechaActual.substr(0, 10) + " " + fechaActual.substr(11) + ".000Z";
      ...
      if (this.state.EstadoCargueBorrador) {
        for (let i = 0; i < this.state.ListaPedido.length; i++) {
          if (this.state.ListaPedido[i].ArrayMedicamentos) {
            if (
              this.state.ListaPedido[i].ArrayMedicamentos.Productos.length > 0
            ) {
              arrayDisensacion.push(
                this.state.ListaPedido[i].ArrayMedicamentos
              );
            }
          } else {
      ...
      for (let i = 0; i < arrayDisensacion.length; i++) {
        ...
        ObjPeticion.IdentificacionUsuario =
          arrayDisensacion[i].IdentificacionUsuario;
        ObjPeticion.IdDetallePrescripcion =
          arrayDisensacion[i].IdDetallePrescripcion;
        ...
        for (let j = 0; j < arrayDisensacion[i].Productos.length; j++) {
          ...
          objMedicamento.IdDetallePrescripcion =
            arrayDisensacion[i].IdDetallePrescripcion;
          let Diguito = "";
  ...
  Imprimir = () => {
    this.setState(state => ({
      ...state,
      EstadoImpresion: !this.state.EstadoImpresion
    }));
  };
  ...
  GuradarDispensacionIncompleta = Mensaje => {
    ...
    for (let i = 0; i < objDisensacion.ArrayMedicamentos.length; i++) {
      ...
      for (
        let j = 0;
        j < objDisensacion.ArrayMedicamentos[i].Productos.length;
        j++
      ) {
        let estado = false;
        ArrayTemp = [];
        for (let k = 0; k < this.state.ArrayEntregados.length; k++) {
          for (
            let p = 0;
            p < this.state.ArrayEntregados[k].lstMedicamentos.length;
            p++
          ) {
            if (
              objDisensacion.ArrayMedicamentos[i].Productos[j].Codigo ===
                this.state.ArrayEntregados[k].lstMedicamentos[p].Codigo &&
              objDisensacion.ArrayMedicamentos[i].Productos[j].Lote ===
                this.state.ArrayEntregados[k].lstMedicamentos[p].Lote
            ) {
              estado = true;
            }
          }
  ...
  onClick = Datos => {
    ...
    for (let i = 0; i < arrayBorrador.length; i++) {
      ...
      for (let k = 0; k < this.state.ListaPedido.length; k++) {
        if (
          Number(arrayBorrador[i].ArrayMedicamentos.IdDetallePrescripcion) ===
          this.state.ListaPedido[k].IdDetallePrescripcion
        ) {
  ...
  ValorAprovechamiento = event => {
    if (event.target) {
      ...
      if (listaTemp[event.target.id].Aprovechamiento) {
        Pendiente =
          Pendiente + Number(listaTemp[event.target.id].Aprovechamiento);
      }
        ...
        if (event.target.value) {
          listaTemp[event.target.id].ArrayMedicamentos.Pendiente =
            ...
            (parseInt(
              ...
              parseInt(listaTemp[event.target.id].ArrayMedicamentos.Entrega));
        } else {
          listaTemp[event.target.id].ArrayMedicamentos.Pendiente =
            listaTemp[event.target.id].ArrayMedicamentos.Cantidad -
            listaTemp[event.target.id].ArrayMedicamentos.Entrega;
      ...
      if (array) {
        fetch(ApiSios + "/consultarStockMedicamentos", {
          ...
          body: JSON.stringify({
            Filtro: array,
            Software: "ZEUS",
            Parametro1: this.state.CodigoBodega
          }),
          headers: {
            "Content-Type": "application/json; charset=UTF-8",
            Accept: "application/json"
          ...
          .then(data => {
            ...
            if (data.error || data.Datos == undefined) {
              ...
              this.setState(state => ({
                ...state,
                ValueMedicamento: this.state.ListaPedido[IndexMedicamento]
              }));
              this.state.ListaMedicamentos = LstDatos;
              if (this.state.EstadoCargueBorrador) {
                if (
                  this.state.ListaPedido[IndexMedicamento].ArrayMedicamentos
                ) {
                  if (
                    this.state.ListaPedido[IndexMedicamento].ArrayMedicamentos
                      .Productos
                    ...
                    this.setState(state => ({
                      ...state,
                      MedicamentoZeus: this.state.ListaPedido[IndexMedicamento]
                        .ArrayMedicamentos.Productos
                    }));
                  }
                } else {
                  if (this.state.ListaPedido[IndexMedicamento].Productos) {
  ...
  QuitarMedicamento = index => {
    ...
    this.state.ListaPedido[index].Pendiente = this.state.ListaPedido[
      index
    ].Cantidad;
    this.setState(state => ({}));
  };
  ...
  CerrarModalArchivacion = () => {
    this.setState(state => ({
      ...state,
      ModelEliminacion: false
    }));
  };
  EliminarMedicamento = data => {
    this.setState(state => ({
      ...state,
      ModelEliminacion: true
    }));
    this.setState(state => ({
      ...state,
      objetoEliminacion: data
    }));
  };
  ...
  procesarMedicamentoEliminar = () => {
    ...
    let nuevoArray = this.state.ListaPedido.filter(
      elemento => elemento._id !== IdMecicamento
    );
    this.setState(state => ({
      ...state,
      ListaPedido: nuevoArray
    }));
  ...
  ActualizarUnidad = () => {
    ...
    fetch(gsUrlApi + "/prescripciones/ActualizarUnidad/", {
      method: "POST",
      body: JSON.stringify({
        IdPrescripcion: this.state.PrescripcionesSelect?.value,
        unidad: this.state.UnidadFuncionalSelect
      }),
      headers: {
        "Content-Type": "application/json; charset=UTF-8",
        Accept: "application/json"
  ...
  consultarEstadoPaciente = async () => {
    fetch(
      "http://10.10.1.15/ArmadoCuentas/api/ArmadoCuentas/obtenerDatosSios",
      {
        method: "POST",
        body: JSON.stringify({
          Parametro1: Caso,
          Consulta: "Farmacia_ConsultaEstadoCaso"
        }),
        headers: {
          "Content-Type": "application/json; charset=UTF-8",
          Accept: "application/json",
      ...
      .then(data => {
        if (data?.Datos?.length > 0) {
          ...
          if (data?.Datos[0].EstadoCaso == true) {
            mensaje =
              mensaje +
                " aun se encuentra en el centro de atención, La unidad funcional actual es " +
                DataUnidades +
                " Sede: " +
                this.state.arraySedes.find(
                  dt =>
                    dt.id?.toString() ===
                    data?.Datos[0]?.CentroAtencion?.toString()
                )?.Nombre || "";
          } else {
            mensaje =
              mensaje +
              " ya NO se encuentra en el centro de atención, La ultima unidad funcional del paciente fue " +
              DataUnidades;
          }
          confirmAlert({
            title: "Información",
            ...
            <div className="row mb-2">
              <div className="col-md-6 row">
                <div className="font-weight-bold">
                  ...
                  {this.state.DataPaciente.NombrePaciente}-
                </div>
                <div className="">{this.state.DataPaciente.LogoSexo}</div>-
                {this.state.DataPaciente.Edad + "  Años"}
              </div>
              <div className="col-md-2 row">
                <div className="font-weight-bold">Piso:</div>
                <div className="ml-2">{piso}</div>
              </div>
              <div className="col-md-4 row">
                <div className="font-weight-bold">Unidad Funcional:</div>
                <div className="ml-2">{UnidadFuncionalIN}</div>
              </div>
            </div>

            <div className="row mt-3 mb-2">
              <div className="col-md-3 pl-3 row">
                <div className="font-weight-bold">Erp:</div>
                <div className=" ml-2">{Erp}</div>
              </div>
              <div className="col-md-6 row">
                <div className="font-weight-bold">Pabellon:</div>
                <div className=" ml-2 mr-2">{pabellon}</div>
                <i
                  style={{ color: "green" }}
                  className="fas fa-procedures mt-1 ml-2 mr-1"
                >
                  :
                </i>
              ...
              <div className="col-md-3 row">
                <div className="font-weight-bold">Caso:</div>
                <div className=" ml-2">{Caso}</div>
              </div>
            </div>
          </div>
          <div className="row">
              ...
              <div>
                ...
                >
                  <ReactToPrint
                    id="Impresion"
                    content={
                      this.state.EstadoImprimir ? this.reactToPrintContent2 : ""
                    }
                    documentTitle="Dispensacion"
                    onAfterPrint={
                      this.state.EstadoImprimir ? this.handleAfterPrint : ""
                    }
                    onBeforeGetContent={
                      this.state.EstadoImprimir
                        ? this.handleOnBeforeGetContent2
                        : ""
                    }
                    onBeforePrint={
                      this.state.EstadoImprimir ? this.handleBeforePrint : ""
                    }
                    removeAfterPrint
                ...
                <div hidden={true}>
                  <Constancia
                    ...
                    CodigoUnidadFuncionalImpr={
                      this.state.CodigoUnidadFuncionalImpr
                    }
                ...
                <Table hover className="mb-0 border">
                  <thead className="">
                    <tr>
                      ...
                      <th>Entrega</th>
                      <th>Pendiente</th>
                      {this.state.BloqueoDispensacion ? "" : <th></th>}
                      <th>Aprovecha..</th>
                      <th></th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={i} style={{ backgroundColor: e.ColorBorder }}>
                          <td style={{ width: "120px" }}>
                            <div className="">
                              {e.FechaRegistro.substring(0, 10)} Hora:{" "}
                              {e.FechaRegistro.substring(11, 16)}
                            </div>
                            <div className="">
                              {"Tipo: "}
                              {e.Pedido24horas == "true" ? (
                                <label
                          ...
                          <td>
                            ...
                            <div>
                              ...
                              {e.ArrayMedicamentos
                                ? e.ArrayMedicamentos.Posologia.replace(
                                    e.ArrayMedicamentos.Medicamento,
                                    ""
                                  )
                                : e.Posologia.replace(e.Medicamento, "")}
                            </div>
                          </td>
                          <td>
                            {e.ArrayMedicamentos
                              ? e.ArrayMedicamentos.NombreUnidadFuncional
                              : e.NombreUnidadFuncional}
                          </td>
                          <td>
                            {e.ArrayMedicamentos
                              ? e.ArrayMedicamentos.NombreUsuario
                              : e.NombreUsuario}
                          </td>
                                  ...
                                  e.ArrayMedicamentos
                                    ? () =>
                                        this.ListarProductos(
                                          e.ArrayMedicamentos.CodigoServicio,
                                          i,
                                          e.ArrayMedicamentos.Generico
                                        )
                                    : () =>
                          ...
                          >
                            <div className="row">
                              <input
                                ...
                                onChange={
                                  e.ArrayMedicamentos
                                    ? this.ValorAprovechamientoBorrador
                                    : this.ValorAprovechamiento
                                }
                                disabled={
                                  e.ArrayMedicamentos
                                    ? e.ArrayMedicamentos.EstadoAprovechamiento
                                    : e.EstadoAprovechamiento
                                }
                                value={
                                  e.ArrayMedicamentos
                                    ? e.ArrayMedicamentos.Aprovechamiento
                                    : e.Aprovechamiento
                                }
                                type="number"
                              />
                              ...
                              {e.ArrayMedicamentos ? (
                                <>
                                  ...
                                  {e.ArrayMedicamentos.Entrega !== 0 ? (
                                    ...
                                    >
                                      <i
                                        className="fas m-1 text-danger fa-eraser cursor-pointer"
                                        style={{ fontSize: "20px" }}
                                        disabled="true"
                                        onClick={() =>
                                          this.QuitarMedicamento(i)
                                        }
                                      ></i>
                                    </div>
                                  ) : (
          ...
          >
            ...
            <ModalBody>
              <FormModal
                ...
                handleFormSubmit={data => this.handleFormSubmit(data)}
                ListaMedicamentos={this.state.ListaMedicamentos}
                ModificarListaMedicamentos={data =>
                  this.ModificarListaMedicamentos(data)
                }
              ...
              <div className="row">
                <div className="col-md-6">
                  <span>
                    Solicitados: ({this.state.ValueMedicamento.Cantidad}){" "}
                  </span>
                </div>
              </div>
            </ModalBody>
            <ModalFooter>
              ...
              >
                <lable>
                  Sede: {this.state.Sede} , UnidadFuncional:{" "}
                  {this.props.data.CodigoUnidadFuncional} -{" "}
                  {this.state.DataPaciente.UnidadFuncional}, Bodega:{" "}
                  {this.state.CodigoBodega} - {this.state.NombreBodega},
                  IdArticulo :({this.state.ArrayMedicamentoList})
                </lable>
              </div>
              <div className="row">
                <i
