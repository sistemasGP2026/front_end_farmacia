...
class FormModal extends React.Component {
  ...
  handleChange = (event) => {
    ...
    if (event.target) {
      ...
      if (TotalSuma > this.props.Cantidad || event.target.value < 1 || listaTemp[event.target.id].Stock < TotalSuma) {
    ...
    //          if (ArrayMedicamentos[i].Codigo === listaTemp[event.target.id].Codigo && ArrayMedicamentos[i].Costo === listaTemp[event.target.id].Costo) {
    //           if (event.target.value  > listaTemp[event.target.id].Costo) {
    //                   let valorrestante = event.target.value  - listaTemp[event.target.id].Costo;
    ...
    //     //   if (ArrayMedicamentos[i].FechaVencimiento === listaTemp[event.target.id].FechaVencimiento) {
    ...
    //     //   //  listaTemp[event.target.id].Costo 
            
    //     //   }else if (ArrayMedicamentos[i].FechaVencimiento != listaTemp[event.target.id].FechaVencimiento){
    ...
    //     let costocomparacion = toInteger(objMedicamento.Costo);
    //     let cantidadcomparacion = toInteger(event.target.value);

    //     let total = cantidadcomparacion/listaTemp.length
    //     console.log(total)

    //     if (cantidadcomparacion > costocomparacion) {
    //       alert("La cantidad digitada es mayor al stock");
    ...
    //     } else if (cantidadcomparacion < 0) {
    //       alert("Colocar una cantidad mayor o igual a 0");
    //       event.target.value = "0";
    //     } else {
    //       ArrayMedicamentos.push(objMedicamento);
    //       this.props.AgregarMedicamento(ArrayMedicamentos);
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td style={{ width: "119px" }}>
                            <div className="">
                              {e.FechaVencimiento.substring(0, 10)}
                            </div>
                          </td>
                          <td>{e.Stock}</td>
