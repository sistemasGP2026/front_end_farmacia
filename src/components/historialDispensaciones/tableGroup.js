...
import PrintData from '../dispensaciones/PrintData';
...
class TablaModelo extends React.Component {
  ...
  checkedPaciente = e => {
    ...
    const ArrayTemp = this.state.SelectedMedicamento ? this.state.SelectedMedicamento : [];
    const ArrayTempId = this.state.SelectedIdMedicamento ? this.state.SelectedIdMedicamento : [];
    if (selectedTem.includes(e.target.value)) {
      let index = selectedTem.indexOf(e.target.value);
      if (index > -1) {
        selectedTem.splice(index, 1);
        ...
        <div style={{ padding: '10px' }}>
          <div className="row col-md-12">
            <div className="form-group col-md-3">
              <h5>
                <i className="fas fa-check " /> {this.props.TextoTablaCabecera}
              </h5>
            </div>
            <>
            ...
            {this.props.Userlists.map((row, i) => (
              <>
                <TableRow>
                  ...
                  <TableCell>
                  <div className="row">{row.NombreCentroAtencion}</div>
                    
                     <div className="row ">Entregado por:<div className="ml-2 font-weight-bold">{row.UsuarioFarmacia}</div></div>
                  </TableCell>
                    ...
                    <Collapse in={row.EstadoOpen} timeout="auto" unmountOnExit>
                      <Box margin={1}>
                        ...
                        <Table size="small" aria-label="purchases">
                          ...
                          {this.props.Userlists[i]["ArrayMedicamentos"].map((historyRow) => (
                            <>
                              {historyRow["Productos"].length > 0
                                    ...
                                    <TableRow>
                                      ...
                                      <TableCell className="text-center">
                                        {historyRow.Estado === "Dispensado" ? (
                                          <button type="button" className="text-white text-12" disabled style={{ background: "blueviolet", border: "0px", padding: "5px", borderRadius: "15px" }}>{historyRow.Estado}</button>
                                        ) : (
                                          <button type="button" className="text-12 text-white" disabled style={{ background: "limegreen", border: "0px", padding: "5px", borderRadius: "15px" }}>{historyRow.Estado}</button>
                                        )}
